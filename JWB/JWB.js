...
JWB.setup.load = function() {
	...
	JWB.api.call({
		...
		function analyticsInfo() { // show info about analytics to users informing them about the opt-out
			$('#resultWindow').html('<h1>'+
					JWB.msg('analytics-firstuse')+
				'</h1><p><strong>'+
					JWB.msg('analytics-opt-out', JWB.msg('tab-setup'))+
				'</strong></p><p><i>'+
					JWB.msg('analytics-dismiss', JWB.msg('tab-setup'))+
					' <a href="https://en.wikipedia.org/wiki/User:Joeytje50/JWB#Analytics">'+JWB.msg('analytics-link')+'</a>'+
				'</i></p>');
		}
		...
		if (data["_google_analytics"] === false) {
			...
			if (data["_google_analytics"] === undefined) analyticsInfo();
